...
const form = useForm({
    _method: "patch",
    client_id: props.clientContact.client.id,
    first_name: props.clientContact.first_name,
    last_name: props.clientContact.last_name,
    phone: props.clientContact.phone,
    job_title: props.clientContact.job_title,
    email: props.clientContact.email,
});

const updateClientContact = (clientContact) => {
    form.post(route("client.contacts.update", clientContact), {
        preserveScroll: true,
...
<template>
    <section>
        ...
        <form
            @submit.prevent="updateClientContact(clientContact)"
            class="mt-6 p-4"
        >
